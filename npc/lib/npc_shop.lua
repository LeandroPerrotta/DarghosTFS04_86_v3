...
function NpcShop:addNegotiableItemByName(item_name, buy, sell, subtype, rating)
	local item_id = getItemIdByName(item_name)
	
	if(item_id) then
		self:addNegotiableItem(item_id, buy, sell, subtype, item_name, rating)
	end
end

function NpcShop:addNegotiableItem(item_id, buy, sell, subtype, name, rating)
	table.insert(self.itemList, {id = item_id, subType = subtype or -1, buy = buy, sell = sell, name = name, requireRating = rating})
end

function NpcShop:addNegotiableListItems(list)
	for k,v in pairs(list) do
		...
		if(v.name == nil and v.id == nil) then
			...
			if(not v.id) then
				self:addNegotiableItemByName(v.name, v.buy, v.sell, v.subType, v.requireRating)
			else
				local item_name = v.name or getItemInfo(v.id).name
				self:addNegotiableItem(v.id, v.buy, v.sell, v.subType, item_name, v.requireRating)
			end
		end	
	end
end

function NpcShop:getNegotiableItem(itemid)
...
function NpcShop:onPlayerBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
	
	local item = self:getNegotiableItem(itemid)
	...
	if(self.type == NPC_TRADETYPE_HONOR) then
		...
		table.insert(self.intentions, cid, 
		    {callback = function(...) self:onPlayerConfirmBuyCallback(...) end, args = {cid, itemid, subType, amount, ignoreCap, inBackpacks}})
...
function NpcShop:onPlayerConfirmBuyCallback(args)
	...
	local cid, itemid, subType, amount, ignoreCap, inBackpacks = args[1], args[2], args[3], args[4], args[5], args[6]
...
function NpcShop:onPlayerSell(cid, itemid, subType, amount, ignoreCap, inBackpacks)

end

function NpcShop:onPlayerRequestTrade(cid)
	...
	addEvent(openShopWindow, NPC_DIALOG_INTERVAL, cid, itemList, 
		function(cid, itemid, subType, amount, ignoreCap, inBackpacks) self:onPlayerBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks) end
		, function(cid, itemid, subType, amount, ignoreCap, inBackpacks) self:onPlayerSell(cid, itemid, subType, amount, ignoreCap, inBackpacks) end)	
end
