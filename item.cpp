	...
	ItemAttributes(), id(type)
{
	...
	if(it.wieldPosition == SLOT_HEAD)
	{
		int16_t durability = Items::helmetsDurability[it.m_rarity][it.m_quality];
		setDurability(durability, durability);
	}
	else if(it.wieldPosition == SLOT_ARMOR)
	{
		int16_t durability = Items::chestsDurability[it.m_rarity][it.m_quality];
		setDurability(durability, durability);
	}
	else if(it.wieldPosition == SLOT_LEGS)
	{
		int16_t durability = Items::legsDurability[it.m_rarity][it.m_quality];
		setDurability(durability, durability);
	}
	else if(it.wieldPosition == SLOT_FEET)
	{
		int16_t durability = Items::feetsDurability[it.m_rarity][it.m_quality];
		setDurability(durability, durability);
	}
	else if(it.weaponType == WEAPON_WAND)
	{
		int16_t durability = Items::lightItemsDurability[it.m_rarity];
		setDurability(durability, durability);
	}
	else if((it.weaponType == WEAPON_AXE || it.weaponType == WEAPON_SWORD || it.weaponType == WEAPON_CLUB) && !(it.slotPosition & SLOTP_TWO_HAND))
	{
		int16_t durability = Items::mediumItemsDurability[it.m_rarity];
		setDurability(durability, durability);
	}
	else if(((it.weaponType == WEAPON_AXE || it.weaponType == WEAPON_SWORD || it.weaponType == WEAPON_CLUB) && (it.slotPosition & SLOTP_TWO_HAND)) || it.weaponType == WEAPON_SHIELD)
	{
		int16_t durability = Items::heavyItemsDurability[it.m_rarity];
		setDurability(durability, durability);
	}
	else if(it.weaponType == WEAPON_DIST && (it.slotPosition & SLOTP_TWO_HAND))
	{
		int16_t durability = Items::distanceDurability[it.m_rarity];
		setDurability(durability, durability);
	}
#endif
}
...
#ifdef __DARGHOS_CUSTOM__
		if(it.m_maxDurability > 0 || (item && item->getMaxDurability() > 0))
		{
			if(begin)
			{
				begin = false;
			...
			if(item)
				...
				s << "Dur: " << it.m_durability << "/" << it.m_maxDurability << std::noshowpos;
		}
#endif
...
void Item::onWear(bool onDeath/* = false*/)
{
	...
	if(!onDeath)
	{
		if(random_range(1, 100000) <= g_config.getNumber(ConfigManager::ITEM_DURABILITY_LOSS_CHANCE))
			wearDurability = true;
	}
	else
		wearDurability = true;
	...
	if(wearDurability)
	{
		setDurability(std::max(0, getDurability() - durabilityWear), getMaxDurability());
	}
	...
	if(getDurability() == 0)
	{
		...
		{
			Item* item = NULL;
			if(item = player->getInventoryItem(SLOT_BACKPACK))
			{
				g_game.internalMoveItem(NULL, getParent(), item->getContainer(), INDEX_WHEREEVER, this, getItemCount(), NULL);
			}
		}
	}
}
