...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		bool isWorn() const { return (getMaxDurability() > 0 && getDurability() == 0) ? true : false; }
#endif
...
inline int32_t Item::getDurability() const
{
	...
	return items[id].m_durability;
}

inline int32_t Item::getMaxDurability() const
{
	const int32_t* v = getIntegerAttribute("maxdurability");
	if(v)
		return *v;

	return items[id].m_maxDurability;
}
#endif
