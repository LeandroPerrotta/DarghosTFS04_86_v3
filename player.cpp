...
Item* Player::getWeapon(bool ignoreAmmo)
{
	...
	{
#ifdef __DARGHOS_CUSTOM__
		if(!(item = getEquippedItem((slots_t)slot)) || item->getWeaponType() != WEAPON_DIST || item->isWorn())
#else
...
ItemVector Player::getWeapons() const
{
	...
	{
#ifdef __DARGHOS_CUSTOM__
		if(!(item = getEquippedItem((slots_t)slot)) || item->isWorn())
#else
...
#ifdef __DARGHOS_CUSTOM__
		Item* item = NULL;
		if((item = getInventoryItem((slots_t)i)) && !item->isWorn())
#else
...
#else
		if(!(item = getInventoryItem((slots_t)slot)) || item->getWeaponType() != WEAPON_SHIELD || item->isWorn())
#endif
			continue;
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_ITEMFULLYWORN:
			sendCancel("This item is is fully worn. To use it again you must repair before.");
			break;
#endif
...
#ifdef __DARGHOS_CUSTOM__
		if(item->getMaxDurability() > 0 && item->getDurability() == 0)
			return RET_ITEMFULLYWORN;
#endif
